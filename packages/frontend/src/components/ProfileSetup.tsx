...
const ProfileSetup = ({ walletAddress, onProfileComplete }: ProfileSetupProps) => {
  ...
  return (
    <div className="fixed inset-0 w-full h-full bg-gradient-to-br from-pink-100 via-purple-100 to-blue-100 flex items-center justify-center p-4 overflow-auto">
      <div className="w-full max-w-6xl my-auto">
        {/* Header */}
        <div className="flex items-center justify-between mb-3 flex-wrap gap-3">
          <div className="flex items-center gap-2">
            <div className="w-9 h-9 bg-gradient-to-br from-pink-200 to-purple-300 rounded-full flex items-center justify-center shadow-[inset_3px_3px_10px_rgba(160,100,200,0.3)]">
              <img src={logo} alt="Project Logo" className="object-contain w-[130%] h-[130%]"/>
            </div>
            <h1 className="text-xl font-bold bg-gradient-to-r from-purple-500 via-pink-500 to-blue-500 bg-clip-text text-transparent">Carepanion</h1>
          </div>
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1.5 text-xs text-green-600 bg-white/60 backdrop-blur-sm px-3 py-1.5 rounded-full shadow-[3px_3px_10px_rgba(16,185,129,0.2)] font-semibold">
              <Check className="w-3.5 h-3.5" />
              Wallet Connected
            </div>
            <div className="text-xs text-purple-600 bg-white/60 backdrop-blur-sm px-3 py-1.5 rounded-full font-mono font-semibold shadow-[3px_3px_10px_rgba(160,100,200,0.2)]">
        ...
        <div className="mb-3">
          <div className="flex items-center gap-2 mb-1">
            <div className="h-2 flex-1 bg-gradient-to-r from-pink-300 to-purple-400 rounded-full shadow-[inset_2px_2px_5px_rgba(160,100,200,0.3)]" />
            <div className="h-2 flex-1 bg-white/40 rounded-full" />
            <div className="h-2 flex-1 bg-white/40 rounded-full" />
          </div>
          <p className="text-xs text-purple-600 font-semibold">Step 2 of 3: Profile Setup</p>
        </div>

        {/* Form Container */}
        <div className="bg-white/60 backdrop-blur-sm rounded-3xl p-6 shadow-[20px_20px_60px_rgba(140,100,180,0.5),-20px_-20px_60px_rgba(255,255,255,0.95)] border-2 border-white/70">
          <h2 className="text-xl font-bold text-purple-600 mb-1 text-center">Tell Us About Yourself</h2>
          <p className="text-purple-500 mb-4 text-center text-sm">This helps us understand diverse perspectives and improve our AI models for everyone.</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-3">
              {/* Gender */}
              <div>
                <label className="block text-xs font-bold text-purple-600 mb-2">Gender</label>
                <div className="grid grid-cols-2 gap-2">
                  {genderOptions.map((option) => (
                    <button
                      ...
                      className={`p-2.5 rounded-xl border-2 transition-all text-xs font-semibold ${
              ...
              {/* Age Bracket */}
              <div>
                <label className="block text-xs font-bold text-purple-600 mb-2">Age Bracket</label>
                <div className="grid grid-cols-4 gap-2">
                  {ageBrackets.map((bracket) => (
                    <button
                      ...
                      disabled={isSubmitting}
                      className={`p-2.5 rounded-xl border-2 transition-all font-semibold text-xs ${
              ...
              {/* Nationality */}
              <div>
                <label className="block text-xs font-bold text-purple-600 mb-2">Nationality</label>
                <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              {/* Hearing Ability */}
              <div>
                <label className="block text-xs font-bold text-purple-600 mb-2">Hearing Ability</label>
                <div className="space-y-1.5">
                  {hearingOptions.map((option) => (
                    <button
                      ...
                      disabled={isSubmitting}
                      className={`w-full p-2.5 rounded-xl border-2 transition-all text-left flex items-center justify-between ${
                    ...
                    >
                      ...
                      {profileData.hearing_ability === option && <Check className="w-4 h-4 text-pink-500" />}
                    </button>
                  ))}
                </div>
              </div>
          ...
          <div className="mt-4">
            <div className="relative">
              ...
              <button
                ...
                className={`relative w-full py-3 rounded-full font-bold text-base transition-all flex items-center justify-center gap-2 ${
